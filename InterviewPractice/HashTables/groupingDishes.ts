...
An array of dishes, where dishes[i] for each valid i contains information about the ith dish: dishes[i][0] 
is the name of the dish, and all the elements after it are the ingredients of that dish. Both the dish name and the ingredient names 
consist of English letters and spaces. It is guaranteed that all dish names are different. It is also guaranteed that the ingredient 
names for any one dish are also pairwise distinct.
...
let fillMyMap=(dishes)=>{
    let map =new Map();
    dishes.forEach((dish)=>{
        let name = dish[0]
        
        for(let i=1; i<dish.length; i++){
            if(map.get(dish[i])==null){
                map.set(dish[i],[]);
                map.get(dish[i]).push(name);
            }else{
                map.get(dish[i]).push(name);
            }
        }
...
let sortMyMap= (map)=>{
    ...
    map.forEach((value,key)=>{
        if(value.length>1){
            ...
            ingredient=ingredient.concat(value.sort());
            //console.log(ingredient);
            ingredientList.push(ingredient);
        }
    });
    console.log(ingredientList);
    return  ingredientList.sort((a, b) => a[0] < b[0] ? -1 : a[0] === b[0] ? 0 : 1);
}


function groupingDishes(dishes: string[][]): string[][] {
    
    let map = fillMyMap(dishes);
    /*map.forEach(function(value, key) {
	console.log(key + " = " + value);
    })*/
    //console.log(sortMyMap(map));
    return sortMyMap(map);
