...
are more than 1 duplicated numbers, return the number for which the second occurrence has a
...
int firstDuplicate(int[] a) {
    int len=a.length;
    for(int i=0; i<len;i++){
        //As the array  can only have numbers from 1 to a.length
        //we read each number and its position of the array will be used
        //to identify if it was already seen or not. the value is only required
        //for the position but for the seen the sign is used
