...
function isCryptSolution(crypt: string[], solution: string[][]): boolean {
    ...
    let myHash = new Map();
    //the has is used to add the values  to an array
    let numArray : number[]= new Array<number>();
    solution.forEach( sol => myHash.set(sol[0], sol[1]));
    
    crypt.forEach(str =>{
        
        let tempString :String='';
        for(var i =0; i<str.length;i++){
            tempString+=myHash.get(str[i]);
        }
        if(tempString.length>1 && tempString[0] === '0')
            return false;
        numArray.push(+tempString);  
    });
